...
std::array<AiBuildingMetric, MetricDefinitions::BUILDING_METRIC_COUNT> MetricDefinitions::aiBuildingMetric = buildMetricArray<B, AiBuildingMetric>({
	...
	{B::TYPE_RES_FOOD,      {[](db_building* b, db_building_level* l) -> float { return b->hasResourceType(ResourceType::FOOD); }}},
	{B::TYPE_RES_WOOD,      {[](db_building* b, db_building_level* l) -> float { return b->hasResourceType(ResourceType::WOOD); }}},
	{B::TYPE_RES_STONE,     {[](db_building* b, db_building_level* l) -> float { return b->hasResourceType(ResourceType::STONE); }}},
	{B::TYPE_RES_GOLD,      {[](db_building* b, db_building_level* l) -> float { return b->hasResourceType(ResourceType::GOLD); }}},
...
db_building_metric::db_building_metric(db_building* dbBuilding, db_building_level* dbLevel) :
	...
	setVector(otherNorm, METRIC_DEFINITIONS.getBuildingOtherIdxs());
	setVector(defenceNorm, METRIC_DEFINITIONS.getBuildingDefenceIdxs());
	setVector(resourceNorm, METRIC_DEFINITIONS.getBuildingResourceIdxs());
	setVector(techNorm, METRIC_DEFINITIONS.getBuildingTechIdxs());
	setVector(unitsNorm, METRIC_DEFINITIONS.getBuildingUnitsIdxs());
	setVector(typesNorm, METRIC_DEFINITIONS.getBuildingTypesIdxs());
}
...
db_unit_metric::db_unit_metric(db_unit* dbUnit, db_unit_level* dbLevel) :
	...
	setVector(typesNorm, METRIC_DEFINITIONS.getUnitTypesIdxs());
}
