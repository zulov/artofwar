...
struct AiUnitMetric : AiMetric {
	...
	AiUnitMetric(const std::function<float(db_unit* unit, db_unit_level* level)>& fn, float weight) : AiMetric(weight), fn(fn) {
	}
};
...
struct AiBuildingMetric : AiMetric {
	...
	AiBuildingMetric(const std::function<float(db_building* building, db_building_level* level)>& fn, float weight) : AiMetric(weight), fn(fn) {
	}
};
